...
export class Storage {
  ...
  getEventDetailPath(eventKey) {
    return path.join(this.baseDir, "event-details", `${eventKey}.json`);
  }

  getRankingPath(eventKey) {
    return path.join(this.baseDir, "rankings", `${eventKey}.json`);
  }
  ...
  getPageCachePath(eventKey) {
    return path.join(this.baseDir, "page-cache", `${eventKey}.json`);
  }
  ...
  readEventDetail(eventKey) {
    return readJson(this.getEventDetailPath(eventKey), null);
  }

  readRanking(eventKey) {
    return readJson(this.getRankingPath(eventKey), null);
  }
  ...
  readPageCache(eventKey) {
    return readJson(this.getPageCachePath(eventKey), null);
  }
  ...
  saveEventDetail(detail) {
    ...
    this.safeSave(this.getEventDetailPath(detail.eventKey), detail);
  }

  saveRanking(ranking) {
    if (!ranking?.eventKey) {
      throw new Error("ranking.eventKey is required.");
    }

    this.safeSave(this.getRankingPath(ranking.eventKey), ranking);
  }
  ...
  savePageCache(pageCache) {
    if (!pageCache?.eventKey) {
      throw new Error("pageCache.eventKey is required.");
    }

    this.safeSave(this.getPageCachePath(pageCache.eventKey), pageCache);
  }
